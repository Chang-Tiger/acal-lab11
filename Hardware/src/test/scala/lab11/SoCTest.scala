...
class top_SoCTest(dut: top_SoC) extends PeekPokeTester(dut) {
  ...
  while (!peek(dut.io.Hcf)) {
    ...
    var DM_rvalid = peek(dut.io.Read_Valid).toInt
    var Master_W_b_Valid = peek(dut.io.Master_W_b_Valid).toInt
    var Slave_W_b_Valid = peek(dut.io.Slave_W_b_Valid).toInt
    ...
    println(
      s"[PC_MEM]${"%8d".format(PC_MEM)} [Inst] ${"%-25s".format(lines(PC_MEM >> 2))} " +
        s"[DM Raddr]${"%8s".format(DM_raddr)} [DM Rdata]${"%8s".format(DM_rdata)} [DM R_valid]${"%1s".format(DM_rvalid)}" +
        s" [DM Wdata]${"%8s".format(DM_wdata)} [Master_W_b_Valid]${"%1s".format(Master_W_b_Valid)}  [Slave_W_b_Valid]${"%1s".format(Slave_W_b_Valid)}"
    )
    println(
